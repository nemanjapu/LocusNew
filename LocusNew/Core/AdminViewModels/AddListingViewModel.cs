...
namespace LocusNew.Core.AdminViewModels
{
    public class AddListingViewModel
    {
        ...
        [Required(ErrorMessage = "Vlasnik je obavezno polje!")]
        [DisplayName("Vlasnik")]
        public int PropertyOwnerId { get; set; }
        public IEnumerable<PropertyOwner> PropertyOwners { get; set; }
        [Required(ErrorMessage = "Broj ugovora je obavezno polje!")]
        [Remote("checkListinUniqueCode", "ListingsManager", AdditionalFields = "", ErrorMessage = "Nekretnina s ovim brojem ugovora već postoji.")]
        ...
        {
            Id = new int();
            ContractFrom = DateTime.Now.ToString("dd.MM.yyyy");
            ContractTo = DateTime.Now.ToString("dd.MM.yyyy");
            MetaKeyWords = "Prodaja, kupovina, stan, kuca, zemljište, parcela";
            Rooms = 0;
            Bedrooms = 0;
            Bathrooms = 0;
            SquareMeters = 0;
